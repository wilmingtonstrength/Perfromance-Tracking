...
function RowTimeInput({ value, onChange }) {
  ...
  useEffect(() => {
    if (!initialized && value !== '' && value !== undefined && value !== null) {
      const total = parseFloat(value);
      if (!isNaN(total)) { setMinutes(String(Math.floor(total / 60))); setSeconds(String((total % 60).toFixed(1))); }
      setInitialized(true);
...
function SimpleChart({ data, direction, testDef, onPointClick }) {
  ...
  const formatYLabel = (val) => {
    if (testDef && testDef.feet_inches) return formatFeetInches(val);
    if (testDef && testDef.row_time) return formatRowTime(val);
    ...
    <div style={{ padding: '20px 0' }}>
      <svg viewBox={'-40 -15 ' + (width + 70) + ' ' + (height + 45)} style={{ width: '100%', height: 280 }}>
        {yLabels.map((val, i) => {
          ...
          return (<g key={i}><line x1={0} y1={y} x2={width} y2={y} stroke="rgba(255,255,255,0.08)" strokeWidth="1" /><text x={-8} y={y + 4} fill="#888" fontSize="9" textAnchor="end">{formatYLabel(val)}</text></g>);
        })}
        ...
        {points.map((p, i) => (
          <g key={i} style={{ cursor: onPointClick ? 'pointer' : 'default' }} onClick={() => onPointClick && onPointClick(p)}>
...
export default function App() {
  ...
  // Group tests by category from database
  const testsByCategory = {};
  testDefs.forEach(t => {
    const cat = t.category_label || t.category;
  ...
  const updateResult = async (resultId, updates) => {
    ...
    const cv = testDef && testDef.convert_formula ? applyConversion(testDef, updates.rawValue) : updates.rawValue;
  ...
  const logResults = async (resultsToLog) => {
    ...
    for (const result of resultsToLog) {
      ...
      if (!isPR && testDef) {
        const best = testDef.direction === 'higher' ? Math.max(...prev.map(r => parseFloat(r.converted_value))) : Math.min(...prev.map(r => parseFloat(r.converted_value)));
        isPR = testDef.direction === 'higher' ? result.convertedValue > best : result.convertedValue < best;
      }
  ...
  const getPR = (athleteId, testId) => {
    ...
    return testDef.direction === 'higher' ? Math.max(...ar.map(r => parseFloat(r.converted_value))) : Math.min(...ar.map(r => parseFloat(r.converted_value)));
  };
...
function TestEntryPage({ athletes, logResults, getPR, getTestById, getTestsForType }) {
  ...
  const addAthleteRow = (athleteId) => {
    ...
    const values = {}; selectedTests.forEach(tid => { values[tid] = ''; });
  ...
  const handleSubmit = async () => {
    ...
    const summary = logged.map(r => {
      ...
      return { athlete: (a ? a.first_name + ' ' + a.last_name : 'Unknown'), test: t ? t.name : r.test_id, value: r.converted_value, testId: r.test_id, testDef: t, unit: t ? (t.display_unit || t.unit) : '', isPR: r.is_pr };
    });
          ...
          <div style={{ background: 'rgba(0,255,136,0.1)', borderRadius: 12, padding: 24, border: '1px solid rgba(0,255,136,0.4)', marginBottom: 16 }}>
            ...
            {prResults.map((r, i) => (
              <div key={i} style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ color: '#00ff88', fontWeight: 800, fontSize: 18 }}>{formatResultWithUnit(r.testDef, r.value)}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 24, border: '1px solid rgba(255,255,255,0.1)', marginBottom: 24 }}>
            ...
            {nonPRResults.map((r, i) => (
              <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ color: '#00d4ff' }}>{formatResultWithUnit(r.testDef, r.value)}</span>
              </div>
            ))}
          </div>
        )}
            ...
            <div style={{ overflowX: 'auto' }}>
              <div style={{ display: 'flex', gap: 8, padding: '0 0 8px 0', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: 8, minWidth: 'fit-content' }}>
                ...
                {selectedTests.map(tid => {
                  ...
                  const headerUnit = t && t.allow_kg && useKg ? 'kg' : '';
                  return (
                    <div key={tid} style={{ minWidth: (t && t.feet_inches) ? 130 : (t && t.row_time) ? 120 : 100, flex: 1, fontSize: 11, color: entryMode === 'adult' ? '#FFA500' : '#00d4ff', textTransform: 'uppercase', letterSpacing: 1, textAlign: 'center' }}>
                      {t ? t.name : tid}
                ...
                return (
                  <div key={row.athleteId} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', minWidth: 'fit-content' }}>
                    ...
                    {selectedTests.map(tid => {
                      ...
                      const useFtIn = t && t.feet_inches;
                      const isRowTest = t && t.row_time;
...
function AthletesPage({ athletes, addAthlete, updateAthlete, deleteAthlete, results, getPR, getTestById, getTestsForType, testDefs, deleteResult, updateResult }) {
  ...
  const startEditInfo = () => {
    if (!athlete) return;
    setFirstName(athlete.first_name); setLastName(athlete.last_name);
    setBirthday(athlete.birthday ? String(athlete.birthday).slice(0, 10) : '');
    setGender(athlete.gender || 'Male'); setEmail(athlete.email || ''); setPhone(athlete.phone || '');
    setStatus(athlete.status || 'Active'); setType(athlete.type || 'athlete');
    setEditingInfo(true);
  };

  const saveEditInfo = () => {
    updateAthlete(selectedAthlete, { firstName, lastName, birthday, gender, email, phone, status, type });
  ...
  // Test history for selected athlete
  ...
  const chartData = selectedTest && selectedAthlete
    ? athleteResults.filter(r => r.test_id === selectedTest).sort((a, b) => new Date(a.test_date) - new Date(b.test_date)).map(r => ({ date: new Date(r.test_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), value: parseFloat(r.converted_value), id: r.id }))
    : [];
  ...
  return (
    <div>
      ...
      {showAddForm && (
        <form onSubmit={handleAdd} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 24, marginBottom: 24, border: '1px solid rgba(0,212,255,0.3)' }}>
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 12 }}>
            <button type="submit" style={{ padding: '12px 32px', background: 'linear-gradient(135deg, #00ff88 0%, #00cc6a 100%)', border: 'none', borderRadius: 8, color: '#0a1628', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>Add Person</button>
            <button type="button" onClick={() => { setShowAddForm(false); resetForm(); }} style={{ padding: '12px 32px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>Cancel</button>
          </div>
        </form>
      )}
        ...
        <>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {['all', 'athlete', 'adult'].map(t => (
                <button key={t} onClick={() => setFilterType(t)} style={{ padding: '10px 16px', background: filterType === t ? 'rgba(0,212,255,0.2)' : 'rgba(255,255,255,0.05)', border: filterType === t ? '1px solid #00d4ff' : '1px solid rgba(255,255,255,0.15)', borderRadius: 6, color: filterType === t ? '#00d4ff' : '#aaa', cursor: 'pointer', fontSize: 13, fontWeight: filterType === t ? 600 : 400 }}>
                  {t === 'all' ? 'All' : t === 'athlete' ? 'Youth' : 'Adults'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
            {filteredAthletes.map(a => {
              ...
              return (
                <div key={a.id} onClick={() => handleSelectAthlete(a.id)} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 20, border: `1px solid ${isAd ? 'rgba(255,165,0,0.15)' : 'rgba(255,255,255,0.1)'}`, cursor: 'pointer', transition: 'all 0.15s' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                    <div>
                      ...
                      <p style={{ margin: '4px 0 0 0', color: '#888', fontSize: 14 }}>{age && (age + ' yrs')}{a.gender && (' · ' + a.gender)}</p>
                    </div>
                    <span style={{ padding: '4px 10px', background: (a.status === 'Active' || a.status === 'active') ? 'rgba(0,255,136,0.2)' : 'rgba(255,255,255,0.1)', color: (a.status === 'Active' || a.status === 'active') ? '#00ff88' : '#888', borderRadius: 4, fontSize: 12, fontWeight: 600 }}>{a.status}</span>
                  </div>
      ...
      {athlete && (
        <div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 24, marginBottom: 24, border: `1px solid ${isAdult ? 'rgba(255,165,0,0.15)' : 'rgba(255,255,255,0.1)'}` }}>
            {editingInfo ? (
              <div>
                <h3 style={{ margin: '0 0 16px 0', color: '#00d4ff' }}>Edit Info</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
                  <input type="text" placeholder="First Name" value={firstName} onChange={(e) => setFirstName(e.target.value)} style={iStyle} />
                  <input type="text" placeholder="Last Name" value={lastName} onChange={(e) => setLastName(e.target.value)} style={iStyle} />
                  <div><label style={{ display: 'block', marginBottom: 4, fontSize: 12, color: '#888' }}>Birthday</label><input type="date" value={birthday} onChange={(e) => setBirthday(e.target.value)} style={{ width: '100%', ...iStyle }} /></div>
                  <select value={gender} onChange={(e) => setGender(e.target.value)} style={iStyle}><option>Male</option><option>Female</option></select>
                  <select value={status} onChange={(e) => setStatus(e.target.value)} style={iStyle}><option>Active</option><option>Inactive</option></select>
                  <select value={type} onChange={(e) => setType(e.target.value)} style={iStyle}><option value="athlete">Youth Athlete</option><option value="adult">Adult Client</option></select>
                </div>
                <div style={{ marginTop: 16, display: 'flex', gap: 12 }}>
                  <button onClick={saveEditInfo} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #00ff88 0%, #00cc6a 100%)', border: 'none', borderRadius: 8, color: '#0a1628', fontWeight: 700, cursor: 'pointer' }}>Save</button>
                  <button onClick={() => setEditingInfo(false)} style={{ padding: '10px 24px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>Cancel</button>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: 16 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
                    <h2 style={{ margin: 0, fontSize: 28, fontFamily: "'Archivo Black', sans-serif" }}>{athlete.first_name} {athlete.last_name}</h2>
                    {isAdult && <span style={{ fontSize: 12, background: 'rgba(255,165,0,0.2)', color: '#FFA500', padding: '3px 10px', borderRadius: 10, fontWeight: 600 }}>ADULT</span>}
                  </div>
                  <p style={{ margin: 0, color: '#888', fontSize: 14 }}>
                    {calculateAge(athlete.birthday) && (calculateAge(athlete.birthday) + ' yrs')}
                    {athlete.gender && (' · ' + athlete.gender)}
                    {' · ' + athleteResults.length + ' tests'}
                    {' · ' + athleteResults.filter(r => r.is_pr).length + ' PRs'}
                  </p>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={startEditInfo} style={{ padding: '8px 16px', background: 'rgba(0,212,255,0.15)', border: '1px solid rgba(0,212,255,0.3)', borderRadius: 6, color: '#00d4ff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Edit Info</button>
                  <button onClick={() => { deleteAthlete(athlete.id, `${athlete.first_name} ${athlete.last_name}`); setSelectedAthlete(null); }} style={{ padding: '8px 16px', background: 'rgba(255,100,100,0.15)', border: '1px solid rgba(255,100,100,0.3)', borderRadius: 6, color: '#ff6666', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Delete</button>
                </div>
              </div>
            )}
          </div>
          ...
          {profileTab === 'prs' && (
            <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 24, border: '1px solid rgba(255,255,255,0.1)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 20 }}>
                {Object.entries(testSet).map(([catLabel, tests]) => (
                  <div key={catLabel}>
                    <h4 style={{ color: isAdult ? '#FFA500' : '#00d4ff', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>{catLabel}</h4>
                    {tests.map(t => {
                      ...
                      return (
                        <div key={t.id} onClick={() => { setProfileTab('progress'); setSelectedTest(t.id); }} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: 14, cursor: 'pointer' }}>
                          ...
                          <span style={{ fontWeight: 600, color: pr !== null ? '#00ff88' : '#555' }}>{pr !== null ? formatResultWithUnit(t, pr) : '-'}</span>
                        </div>
                      );
                    })}
                  </div>
          ...
          {profileTab === 'history' && (
            <div>
              <div style={{ marginBottom: 16 }}>
                <select value={historyFilter} onChange={(e) => setHistoryFilter(e.target.value)} style={{ ...iStyle, width: 260 }}>
                  <option value="">All Tests</option>
                  {Object.entries(testSet).map(([catLabel, tests]) => (
                    <optgroup key={catLabel} label={catLabel}>{tests.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</optgroup>
                  ))}
                </select>
              </div>
              {sortedHistory.length > 0 ? (
                      ...
                      <div key={r.id} style={{ display: 'flex', alignItems: 'center', padding: '12px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)', gap: 12, flexWrap: 'wrap' }}>
                        {isEd ? (
                          <>
                            ...
                            <span style={{ color: '#00d4ff', fontSize: 14, fontWeight: 600 }}>{t?.name || r.test_id}</span>
                            ...
                            {t && t.feet_inches && editValue && <span style={{ color: '#888', fontSize: 12 }}>= {formatFeetInches(parseFloat(editValue))}</span>}
                            {t && t.row_time && editValue && <span style={{ color: '#888', fontSize: 12 }}>= {formatRowTime(parseFloat(editValue))}</span>}
                            <button onClick={() => handleSaveResult(r)} style={{ padding: '6px 12px', background: 'rgba(0,255,136,0.3)', border: 'none', borderRadius: 4, color: '#00ff88', cursor: 'pointer', fontSize: 12 }}>Save</button>
                          ...
                          <>
                            ...
                            <div style={{ flex: 1, color: '#00d4ff', fontSize: 14, fontWeight: 600 }}>{t?.name || r.test_id}</div>
                            <div style={{ fontWeight: 700, color: r.is_pr ? '#ffd700' : '#00ff88' }}>
                              {t ? formatResultWithUnit(t, r.converted_value) : r.converted_value}
                              {r.is_pr && ' 🏆'}
                            </div>
                            <button onClick={() => handleEditResult(r)} style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 4, color: '#aaa', cursor: 'pointer', fontSize: 12 }}>Edit</button>
                ...
                <div style={{ textAlign: 'center', padding: 48, color: '#666' }}>No results found.</div>
              )}
            </div>
          )}
          ...
          {profileTab === 'progress' && (
            <div>
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', marginBottom: 8, fontSize: 14, color: '#aaa' }}>Select Test</label>
                <select value={selectedTest} onChange={(e) => setSelectedTest(e.target.value)} style={{ ...iStyle, width: 280 }}>
              ...
              {chartData.length > 0 && testDef ? (
                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 24, border: '1px solid rgba(255,255,255,0.1)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
                    <h3 style={{ margin: 0, fontSize: 20 }}>{testDef.name} Progress</h3>
                    {getPR(selectedAthlete, selectedTest) !== null && (
                      <div style={{ padding: '8px 16px', background: 'rgba(0,255,136,0.2)', borderRadius: 8, color: '#00ff88', fontWeight: 700 }}>
                        PR: {formatResultWithUnit(testDef, getPR(selectedAthlete, selectedTest))}
                      </div>
                    )}
                  </div>
                  <SimpleChart data={chartData} direction={testDef.direction} testDef={testDef} onPointClick={(p) => { setProfileTab('history'); setHistoryFilter(selectedTest); }} />
                  <div style={{ fontSize: 13, color: '#666', marginTop: 4 }}>Click chart to view/edit individual results</div>
                </div>
              ) : selectedTest ? (
                <div style={{ textAlign: 'center', padding: 48, color: '#666' }}>No data yet for {testDef?.name}</div>
              ) : (
                <div style={{ textAlign: 'center', padding: 48, color: '#666' }}>Select a test above to view progress</div>
              )}
            </div>
          )}

          {/* ATHLETE SCORE TAB */}
          {profileTab === 'score' && !isAdult && (
            <div style={{ background: 'rgba(168,85,247,0.06)', borderRadius: 12, padding: 24, border: '1px solid rgba(168,85,247,0.25)' }}>
              {athleteScore ? (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 32, marginBottom: 28, flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 200 }}>
                      ...
                      <div style={{ fontSize: 12, color: '#666', lineHeight: 1.6 }}>Score = percentile rank · 50 = gym average · 90+ = elite</div>
                    </div>
                  </div>
                  <div style={{ marginBottom: 28 }}>
                    <div style={{ height: 12, background: 'rgba(255,255,255,0.08)', borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
                      <div style={{ height: '100%', width: `${athleteScore.score}%`, background: `linear-gradient(90deg, #7c3aed, ${scoreLabel(athleteScore.score).color})`, borderRadius: 6 }} />
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
                    {athleteScore.breakdown.map(item => {
                      ...
                      const displayVal = item.testId === 'max_velocity' ? item.best.toFixed(1) + ' MPH' : item.unit ? item.best.toFixed(item.unit === 'sec' ? 2 : (item.unit === 'in' ? 1 : 0)) + ' ' + item.unit : item.best.toFixed(2);
                      return (
                    ...
                    {TSA_TEST_IDS.filter(t => !athleteScore.breakdown.find(b => b.testId === t.id)).map(t => (
                ...
                <div style={{ textAlign: 'center', padding: 32, color: '#666' }}>
                  ...
                  <p style={{ fontSize: 13 }}>Needs at least one result in 1 of the 7 scored tests.</p>
                </div>
              )}
            </div>
          )}
      ...
      {!selectedAthlete && !showAddForm && filteredAthletes.length === 0 && (
        <div style={{ textAlign: 'center', padding: 48, color: '#666' }}>No athletes found matching your search.</div>
      )}
    </div>
  );
}
...
function RankingsPage({ athletes, results }) {
  ...
  const scored = youthAthletes
    .map(a => { const s = calculateAthleteScore(a.id, athletes, results); return s ? { athlete: a, ...s } : null; })
    .filter(Boolean)
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: '#888' }}>{scored.length} of {youthAthletes.length} athletes scored</p>
      </div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 28, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 13, color: '#666' }}>Min tests:</span>
          {[1, 2, 3, 4].map(n => (
      ...
      {scored.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 48, color: '#666' }}><p style={{ fontSize: 18 }}>No athletes scored yet.</p></div>
      ) : (
...
function RecordBoardPage({ athletes, results, testDefs, getTestById }) {
  ...
  const buildAdultRecords = (test, genderFilter) => {
    ...
    adultAthletes.forEach(a => {
      ...
      if (!matches) return;
      let best = null;
      const rollupIds = test.id === '_overhead' ? ['press','push_press','jerk','overhead'] : null;
      if (rollupIds) {
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener('visibilitychange', handleVisibility); if (wakeLockRef.current) { wakeLockRef.current.release().catch(() => {}); } };
  }, [tvMode]);
  ...
  useEffect(() => {
    if (!tvMode) return;
    const recalc = () => {
      const content = tvContentRef.current; const container = tvContainerRef.current;
      if (!content || !container) return;
      content.style.transform = 'none';
      content.style.width = container.clientWidth + 'px';
      const naturalH = content.scrollHeight; const availH = container.clientHeight;
  ...
  useEffect(() => {
    if (!tvMode) return;
    const keepAlive = setInterval(() => { document.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, cancelable: true, clientX: Math.floor(Math.random() * 1920), clientY: Math.floor(Math.random() * 1080) })); }, 600000);
  ...
  const renderTestCard = (test, records, isTv) => {
    ...
    const renderRows = (list) => list.length > 0 ? list.map((r, i) => (
      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 6px', margin: '2px 0', borderRadius: 4, ...(i === 0 ? { background: 'linear-gradient(90deg, rgba(200,150,62,0.3) 0%, rgba(200,150,62,0.05) 100%)', borderLeft: '3px solid #C8963E' } : {}) }}>
        <span style={{ fontWeight: 600, fontSize: isTv ? 13 : 14, color: i === 0 ? '#C8963E' : '#e8e8e8' }}>{formatBoardValue(test, r.value)}</span>
  ...
  const renderAdultCard = (test, isTv) => {
    ...
    const renderRows = (list) => list.length > 0 ? list.map((r, i) => (
      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 4px', margin: '2px 0', borderRadius: 4, background: i === 0 ? 'rgba(200,150,62,0.12)' : 'transparent' }}>
        ...
        <span style={{ fontSize: isTv ? 12 : 13, fontWeight: 700, color: rankColors[i], whiteSpace: 'nowrap' }}>{formatBoardValue(test, r.value)}{test.unit !== '' && test.record_board_format === 'round' ? ' ' + (test.display_unit || test.unit) : ''}</span>
      </div>
          ...
          <div ref={tvContentRef} style={{ transformOrigin: 'top left', transform: `scale(${tvScale})` }}>
            {section !== 'adults' && (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${boardSpeed.length}, 1fr)`, gap: 8, marginBottom: 10 }}>
                  {boardSpeed.map(t => renderTestCard(t, speedRecords, true))}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${boardStrength.length}, 1fr)`, gap: 8 }}>
                  {boardStrength.map(t => renderTestCard(t, strengthRecords, true))}
                </div>
              </>
            )}
            {section === 'adults' && (
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(boardAdult.length, 5)}, 1fr)`, gap: 10 }}>
                {boardAdult.map(t => renderAdultCard(t, true))}
              </div>
            )}
          </div>
        </div>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => { setAutoSwitch(true); setTvMode(true); }} style={{ padding: '10px 20px', background: 'linear-gradient(135deg, #C8963E 0%, #A87A2E 100%)', border: 'none', borderRadius: 6, color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 14, letterSpacing: 1 }}>TV Mode</button>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 0, marginBottom: 28, borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)', width: 'fit-content' }}>
        {[{ id: 'boys', label: 'Boys', color: '#00d4ff' }, { id: 'girls', label: 'Girls', color: '#ff6ec7' }, { id: 'adults', label: 'Adults', color: '#C8963E' }].map(s => (
      ...
      {section !== 'adults' && (
        <>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>{boardSpeed.map(t => renderTestCard(t, speedRecords, false))}</div>
          </div>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>{boardStrength.map(t => renderTestCard(t, strengthRecords, false))}</div>
          </div>
        </>
      )}
      {section === 'adults' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>{boardAdult.map(t => renderAdultCard(t, false))}</div>
      )}
    </div>
  );
}
...
function TestSettingsPage({ testDefs, setTestDefs, showNotification }) {
  ...
  const [formUnit, setFormUnit] = useState('sec');
  const [formDirection, setFormDirection] = useState('lower');
  const [formCategory, setFormCategory] = useState('speed');
  const [formCategoryLabel, setFormCategoryLabel] = useState('Speed & Acceleration');
  const [formDisplayUnit, setFormDisplayUnit] = useState('');
  const [formAllowKg, setFormAllowKg] = useState(false);
  const [formFeetInches, setFormFeetInches] = useState(false);
  const [formRowTime, setFormRowTime] = useState(false);
  const [formAthleteType, setFormAthleteType] = useState('athlete');
  const [formShowOnBoard, setFormShowOnBoard] = useState(false);
  const [formBoardSection, setFormBoardSection] = useState('');
  ...
  const resetForm = () => {
    setFormId(''); setFormName(''); setFormUnit('sec'); setFormDirection('lower'); setFormCategory('speed');
    setFormCategoryLabel('Speed & Acceleration'); setFormDisplayUnit(''); setFormAllowKg(false);
    setFormFeetInches(false); setFormRowTime(false); setFormAthleteType('athlete');
    setFormShowOnBoard(false); setFormBoardSection(''); setFormBoardFormat('fixed2');
  };

  const startEdit = (t) => {
    setEditingTest(t.id); setFormId(t.id); setFormName(t.name); setFormUnit(t.unit); setFormDirection(t.direction);
    setFormCategory(t.category); setFormCategoryLabel(t.category_label || ''); setFormDisplayUnit(t.display_unit || '');
    setFormAllowKg(t.allow_kg || false); setFormFeetInches(t.feet_inches || false); setFormRowTime(t.row_time || false);
    setFormAthleteType(t.athlete_type || 'athlete'); setFormShowOnBoard(t.show_on_record_board || false);
    setFormBoardSection(t.record_board_section || ''); setFormBoardFormat(t.record_board_format || 'fixed2');
  ...
  const handleSave = async () => {
    ...
    const testData = {
      name: formName, unit: formUnit, direction: formDirection, category: formCategory,
      category_label: formCategoryLabel || categoryLabels[formCategory], display_unit: formDisplayUnit || null,
      allow_kg: formAllowKg, feet_inches: formFeetInches, row_time: formRowTime,
      athlete_type: formAthleteType, show_on_record_board: formShowOnBoard,
      record_board_section: formShowOnBoard ? formBoardSection : null,
      record_board_format: formBoardFormat,
    };

    if (editingTest) {
      const { error } = await supabase.from('tests').update(testData).eq('id', editingTest);
      if (!error) {
        setTestDefs(testDefs.map(t => t.id === editingTest ? { ...t, ...testData } : t));
        showNotification(formName + ' updated!');
        setEditingTest(null); resetForm();
      ...
      if (data) {
        setTestDefs([...testDefs, data[0]]);
        showNotification(formName + ' added!');
  ...
  const handleDelete = async (testId, testName) => {
    if (!window.confirm(`Delete "${testName}"? This won't delete existing results but the test won't appear in the app anymore.`)) return;
    const { error } = await supabase.from('tests').update({ active: false }).eq('id', testId);
    if (!error) {
      setTestDefs(testDefs.filter(t => t.id !== testId));
      showNotification(testName + ' removed');
    }
  };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
        ...
        <div><label style={{ display: 'block', marginBottom: 4, fontSize: 12, color: '#888' }}>Unit</label>
          <select value={formUnit} onChange={(e) => {
            ...
            if (e.target.value === 'sec') { setFormDirection('lower'); setFormBoardFormat('fixed2'); }
            if (e.target.value === 'lbs') { setFormDirection('higher'); setFormAllowKg(true); setFormBoardFormat('round'); }
            if (e.target.value === 'inches') { setFormDirection('higher'); setFormBoardFormat('fixed1'); }
          }} style={iStyle}>
            <option value="sec">Seconds (time)</option>
            <option value="lbs">Pounds (weight)</option>
            <option value="inches">Inches (distance)</option>
        ...
        <div><label style={{ display: 'block', marginBottom: 4, fontSize: 12, color: '#888' }}>Better is...</label>
          <select value={formDirection} onChange={(e) => setFormDirection(e.target.value)} style={iStyle}>
            <option value="lower">Lower (faster times)</option>
            <option value="higher">Higher (heavier/taller)</option>
          </select>
        </div>
        <div><label style={{ display: 'block', marginBottom: 4, fontSize: 12, color: '#888' }}>Category</label>
          <select value={formCategory} onChange={(e) => { setFormCategory(e.target.value); setFormCategoryLabel(categoryLabels[e.target.value]); }} style={iStyle}>
            <option value="speed">Speed</option>
            <option value="agility">Agility / COD</option>
        ...
        <div><label style={{ display: 'block', marginBottom: 4, fontSize: 12, color: '#888' }}>For</label>
          <select value={formAthleteType} onChange={(e) => setFormAthleteType(e.target.value)} style={iStyle}>
            <option value="athlete">Youth Athletes Only</option>
            <option value="adult">Adult Clients Only</option>
            <option value="both">Both</option>
          </select>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <label style={{ fontSize: 12, color: '#888' }}>Options</label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#aaa', cursor: 'pointer' }}>
            <input type="checkbox" checked={formAllowKg} onChange={(e) => setFormAllowKg(e.target.checked)} /> Allow kg entry
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#aaa', cursor: 'pointer' }}>
            <input type="checkbox" checked={formFeetInches} onChange={(e) => setFormFeetInches(e.target.checked)} /> Feet & inches input
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#aaa', cursor: 'pointer' }}>
            <input type="checkbox" checked={formShowOnBoard} onChange={(e) => setFormShowOnBoard(e.target.checked)} /> Show on record board
          </label>
        </div>
      </div>
      <div style={{ marginTop: 20, display: 'flex', gap: 12 }}>
        <button onClick={handleSave} style={{ padding: '12px 28px', background: 'linear-gradient(135deg, #00ff88 0%, #00cc6a 100%)', border: 'none', borderRadius: 8, color: '#0a1628', fontWeight: 700, cursor: 'pointer', fontSize: 15 }}>{editingTest ? 'Save Changes' : 'Add Test'}</button>
        <button onClick={() => { setEditingTest(null); setShowAddForm(false); resetForm(); }} style={{ padding: '12px 28px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 8, color: '#fff', cursor: 'pointer' }}>Cancel</button>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: 32, marginBottom: 8 }}>Test Settings</h1>
          <p style={{ color: '#888' }}>{testDefs.length} active tests — add, edit, or remove tests here</p>
        </div>
        <button onClick={() => { setShowAddForm(true); setEditingTest(null); resetForm(); }} style={{ padding: '14px 28px', background: 'linear-gradient(135deg, #00d4ff 0%, #0099cc 100%)', border: 'none', borderRadius: 8, color: '#0a1628', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>+ Add Test</button>
      </div>
      ...
      {Object.entries(grouped).map(([catLabel, tests]) => (
        <div key={catLabel} style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {tests.map(t => (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', background: editingTest === t.id ? 'rgba(0,212,255,0.08)' : 'rgba(255,255,255,0.03)', borderRadius: 8, border: editingTest === t.id ? '1px solid rgba(0,212,255,0.3)' : '1px solid rgba(255,255,255,0.07)', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <span style={{ fontWeight: 600, fontSize: 15 }}>{t.name}</span>
                  <span style={{ color: '#666', fontSize: 12, marginLeft: 8 }}>{t.unit} · {t.direction === 'lower' ? '↓ lower is better' : '↑ higher is better'}</span>
                </div>
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  {t.athlete_type === 'adult' && <span style={{ fontSize: 11, background: 'rgba(255,165,0,0.15)', color: '#FFA500', padding: '3px 8px', borderRadius: 4, fontWeight: 600 }}>Adult</span>}
                  {t.athlete_type === 'both' && <span style={{ fontSize: 11, background: 'rgba(0,212,255,0.15)', color: '#00d4ff', padding: '3px 8px', borderRadius: 4, fontWeight: 600 }}>Both</span>}
                </div>
                <button onClick={() => startEdit(t)} style={{ padding: '6px 14px', background: 'rgba(0,212,255,0.15)', border: 'none', borderRadius: 4, color: '#00d4ff', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Edit</button>
