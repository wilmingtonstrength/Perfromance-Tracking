...
const isMultiRepTest = (testDef) => {
  ...
  if (testDef.multi_rep === true) return true;
  if (testDef.id === 'rsa_shuttle') return true;
  return false;
};
const getMultiRepCount = (testDef) => {
  if (!testDef) return 0;
  if (testDef.rep_count) return parseInt(testDef.rep_count);
  if (testDef.id === 'rsa_shuttle') return 6;
  return 0;
};
const computeRsaStats = (reps) => {
  ...
  return { best: parseFloat(best.toFixed(3)), avg: parseFloat(avg.toFixed(3)), drop: parseFloat(drop.toFixed(2)) };
};
...
function exportProgressShareImage({ athleteName, testName, prText, data, direction, testDef, rangeText }) {
  ...
  const plotL = 170, plotR = 950, plotTop = 860, plotBot = 1620; const plotW = plotR - plotL, plotH = plotBot - plotTop;
  const n = data.length;
  const getX = (i) => n === 1 ? plotL + plotW / 2 : plotL + (i / (n - 1)) * plotW;
  const getY = (val) => plotBot - ((val - chartMin) / chartRange) * plotH;
  const points = data.map((d, i) => ({ x: getX(i), y: getY(d.value), value: d.value, date: d.date }));
  const bestValue = direction === 'lower' ? minVal : maxVal;
  const fmtVal = (v) => testDef ? formatTestValueByDef(testDef, v) : v;
  const fmtY = (v) => { if (testDef && testDef.feet_inches) return formatFeetInches(v); if (testDef && testDef.row_time) return formatRowTime(v); return Number.isInteger(v) ? v : v.toFixed(1); };
  const maxDates = Math.min(n, 6); const dStep = Math.max(1, Math.ceil(n / maxDates));
  const showDate = (i) => { if (i === 0 || i === n - 1) return true; if (i % dStep !== 0) return false; return (n - 1 - i) >= dStep; };
...
export default function App() {
  ...
  const logResults = async (resultsToLog) => {
    ...
    for (const result of resultsToLog) {
      ...
      const { data } = await supabase.from('results').insert([insertRow]).select();
...
function TestEntryPage({ athletes, logResults, getPR, getPRResult, getTestById, getTestsForType }) {
  ...
  const updateRepValue = (rowIndex, testId, repIdx, value) => {
    ...
    cur[repIdx] = value;
    nr[rowIndex] = { ...nr[rowIndex], values: { ...nr[rowIndex].values, [testId]: cur } };
    setAthleteRows(nr);
  };
    ...
    athleteRows.forEach(row => {
      selectedTests.forEach(testId => {
        ...
        if (isMultiRepTest(testDef)) {
          ...
          if (!stats) return;
          toLog.push({
            athleteId: row.athleteId, testId, testDate,
            rawValue: stats.avg, convertedValue: stats.avg,
            unit: testDef.unit || 'sec',
                            ...
                            <div>
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(' + repCount + ', 1fr)', gap: 4 }}>
                                {Array.from({ length: repCount }).map((_, idx) => (
                                  <input key={idx} type="number" step="0.01" placeholder={'R' + (idx + 1)} value={arr[idx] || ''} onChange={(e) => updateRepValue(rowIndex, tid, idx, e.target.value)} onWheel={preventScrollChange} style={{ width: '100%', padding: '6px 2px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 4, color: '#fff', fontSize: 12, textAlign: 'center' }} />
                                ))}
                              </div>
                              {live ? (
...
function AthletesPage({ athletes, addAthlete, updateAthlete, deleteAthlete, results, getPR, getPRResult, getTestById, getTestsForType, testDefs, deleteResult, updateResult, getAssessment, saveAssessment, focusAthlete, clearFocusAthlete }) {
  ...
  const [chartStart, setChartStart] = useState(''); const [chartEnd, setChartEnd] = useState('');
  ...
  const testResultsAll = selectedTest && selectedAthlete ? athleteResults.filter(r => r.test_id === selectedTest) : [];
  const inChartRange = (testDate) => { const d = String(testDate).slice(0, 10); if (chartStart && d < chartStart) return false; if (chartEnd && d > chartEnd) return false; return true; };
  const chartData = testResultsAll.filter(r => inChartRange(r.test_date)).sort((a, b) => new Date(a.test_date) - new Date(b.test_date)).map(r => ({ date: new Date(String(r.test_date).slice(0, 10) + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), value: parseFloat(r.converted_value), id: r.id }));
  const fmtDateStr = (d) => d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
  const computeRangeStart = (p) => { if (!p.months && !p.days && !p.ytd) return ''; const now = new Date(); let s; if (p.ytd) s = new Date(now.getFullYear(), 0, 1); else if (p.days) { s = new Date(now); s.setDate(s.getDate() - p.days); } else { s = new Date(now); s.setMonth(s.getMonth() - p.months); } return fmtDateStr(s); };
  const applyChartRange = (p) => { setChartStart(computeRangeStart(p)); setChartEnd(''); };
  const isRangeActive = (p) => chartEnd === '' && chartStart === computeRangeStart(p);
  const chartRangePresets = [{ label: 'All time' }, { label: '30 days', days: 30 }, { label: '3 months', months: 3 }, { label: '6 months', months: 6 }, { label: 'This year', ytd: true }];
  ...
  if (isMultiRepTest(t)) {
    const rsaResults = athleteResults.filter(r => r.test_id === t.id && Array.isArray(r.reps) && r.reps.length > 0);
    let bestAttempt = null;
    if (rsaResults.length > 0) {
      bestAttempt = rsaResults.reduce((best, r) => {
        const ra = r.avg_time !== null && r.avg_time !== undefined ? parseFloat(r.avg_time) : parseFloat(r.converted_value);
        const ba = best ? (best.avg_time !== null && best.avg_time !== undefined ? parseFloat(best.avg_time) : parseFloat(best.converted_value)) : Infinity;
        return ra < ba ? r : best;
      }, null);
    }
    return (
  ...
  return (<div key={t.id} onClick={() => { setProfileTab('progress'); setSelectedTest(t.id); }} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: 14, cursor: 'pointer' }}><span style={{ color: '#aaa' }}>{t.name}</span><span style={{ fontWeight: 600, color: pr !== null ? '#00ff88' : '#555' }}>{pr !== null ? (prR && t.convert_formula ? formatWithRaw(t, pr, prR.raw_value) : formatResultWithUnit(t, pr)) : '-'}</span></div>);
        ...
        {profileTab === 'progress' && (<div><div style={{ marginBottom: 20 }}><label style={{ display: 'block', marginBottom: 8, fontSize: 14, color: '#aaa' }}>Select Test</label><select value={selectedTest} onChange={(e) => setSelectedTest(e.target.value)} style={{ ...iStyle, width: 280 }}><option value="">Choose a test...</option>{Object.entries(testSet).map(([catLabel, tests]) => (<optgroup key={catLabel} label={catLabel}>{tests.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</optgroup>))}</select></div>{selectedTest && testDef && (<div style={{ marginBottom: 20, padding: 16, background: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 16 }}><div><label style={{ display: 'block', marginBottom: 6, fontSize: 13, color: '#aaa' }}>From</label><input type="date" value={chartStart} max={chartEnd || undefined} onChange={(e) => setChartStart(e.target.value)} style={{ padding: '10px 12px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, color: '#fff', fontSize: 14, colorScheme: 'dark' }} /></div><div><label style={{ display: 'block', marginBottom: 6, fontSize: 13, color: '#aaa' }}>To</label><input type="date" value={chartEnd} min={chartStart || undefined} onChange={(e) => setChartEnd(e.target.value)} style={{ padding: '10px 12px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, color: '#fff', fontSize: 14, colorScheme: 'dark' }} /></div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{chartRangePresets.map(p => { const active = isRangeActive(p); return (<button key={p.label} onClick={() => applyChartRange(p)} style={{ padding: '9px 14px', background: active ? 'linear-gradient(135deg, #00d4ff 0%, #0099cc 100%)' : 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.3)', borderRadius: 8, color: active ? '#0a1628' : '#00d4ff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>{p.label}</button>); })}</div></div>)}{chartData.length > 0 && testDef ? (<div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 24, border: '1px solid rgba(255,255,255,0.1)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}><h3 style={{ margin: 0, fontSize: 20 }}>{testDef.name} Progress</h3><div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>{getPR(selectedAthlete, selectedTest) !== null && (() => { const prVal = getPR(selectedAthlete, selectedTest); const prR = getPRResult(selectedAthlete, selectedTest); return (<div style={{ padding: '8px 16px', background: 'rgba(0,255,136,0.2)', borderRadius: 8, color: '#00ff88', fontWeight: 700 }}>PR: {prR && testDef.convert_formula ? formatWithRaw(testDef, prVal, prR.raw_value) : formatResultWithUnit(testDef, prVal)}</div>); })()}<button onClick={() => { const prVal = getPR(selectedAthlete, selectedTest); const prR = prVal !== null ? getPRResult(selectedAthlete, selectedTest) : null; const prText = prVal !== null ? ('PR ' + ((prR && testDef.convert_formula) ? formatWithRaw(testDef, prVal, prR.raw_value) : formatResultWithUnit(testDef, prVal))) : ''; const fmtFull = (d) => new Date(String(d).slice(0, 10) + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }); const inR = testResultsAll.filter(r => inChartRange(r.test_date)).sort((a, b) => new Date(a.test_date) - new Date(b.test_date)); const rangeText = inR.length ? (fmtFull(inR[0].test_date) + ' – ' + fmtFull(inR[inR.length - 1].test_date)) : ''; exportProgressShareImage({ athleteName: athlete.first_name + ' ' + athlete.last_name, testName: testDef.name, prText, data: chartData, direction: testDef.direction, testDef, rangeText }); }} style={{ padding: '10px 18px', background: 'linear-gradient(135deg, #00d4ff 0%, #0099cc 100%)', border: 'none', borderRadius: 8, color: '#0a1628', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>📸 Share Image</button></div></div><SimpleChart data={chartData} direction={testDef.direction} testDef={testDef} onPointClick={() => { setProfileTab('history'); setHistoryFilter(selectedTest); }} /><div style={{ fontSize: 13, color: '#666', marginTop: 4 }}>Showing {chartData.length} of {testResultsAll.length} results · click a point to view/edit</div></div>) : selectedTest ? (<div style={{ textAlign: 'center', padding: 48, color: '#666' }}>{testResultsAll.length > 0 ? 'No results in this date range — adjust the dates or pick “All time” above.' : ('No data yet for ' + (testDef ? testDef.name : 'this test'))}</div>) : (<div style={{ textAlign: 'center', padding: 48, color: '#666' }}>Select a test above to view progress</div>)}</div>)}
